...
Questa sezione è dedicata per i file che verranno richiamati da tutti i file del progetto, quindi prima creiamo la cartella `utils` e i vari file

``` bash
mkdir src/utils/enums
mkdir src/utils/types
mkdir src/utils/auth/jwt
mkdir src/utils/auth/local
mkdir src/utils/auth/providers
mkdir src/utils/auth/user-identity

# GitKeep
touch src/utils/auth/providers/.gitkeep

# Enum e type user
touch src/utils/enums/user.enum.ts
touch src/utils/types/user.type.ts
...
touch src/utils/auth/local/local-strategy.ts
...
L'user lo creiamo più avanti, ma per ora per creare la `jwt strategy` ci serve l'interface e il model dello user, quindi copia questa interface in `src/api/user/user.interface.ts`
`user.interface.ts`
``` ts
...
import { USER_ROLE_ENUM, USER_STATUS_ENUM } from "../../utils/enums/user.enum";
...
Intanto partiamo dal capire a cosa servono questi import scritti così: 
...
2. Estrae il token dalla richiesta (`Authorization: Bearer ...`)
3. Decodifica e verifica il token con una **chiave segreta**
